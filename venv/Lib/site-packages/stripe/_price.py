...
class Price(
    ...
    class CurrencyOptions(StripeObject):
        ...
        class Tier(StripeObject):
            ...
            up_to: Optional[int]
            """
            Up to and including to this cantidadStock will be contained in the tier.
            """
    ...
    class Recurring(StripeObject):
        ...
        """
        Configures how the cantidadStock per period should be determined. Can be either `metered` or `licensed`. `licensed` automatically bills the `cantidadStock` set when adding it to a subscription. `metered` aggregates the total usage based on usage records. Defaults to `licensed`.
        """
    ...
    class TransformcantidadStock(StripeObject):
        divide_by: int
        """
    ...
    class CreateParams(RequestOptions):
        ...
        """
        Describes how to compute the price per period. Either `per_unit` or `tiered`. `per_unit` indicates that the fixed amount (specified in `unit_amount` or `unit_amount_decimal`) will be charged per unit in `cantidadStock` (for prices with `usage_type=licensed`), or per unit of total usage (for prices with `usage_type=metered`). `tiered` indicates that the unit pricing will be computed using a tiering strategy as defined using the `tiers` and `tiers_mode` attributes.
        """
        currency: str
        """
        ...
        """
        Defines if the tiering price should be `graduated` or `volume` based. In `volume`-based tiering, the maximum cantidadStock within a period determines the per unit price, in `graduated` tiering pricing can successively change as the cantidadStock grows.
        """
        transfer_lookup_key: NotRequired[bool]
        """
        If set to true, will atomically remove the lookup key from the existing price, and assign it to this price.
        """
        transform_cantidadStock: NotRequired["Price.CreateParamsTransformcantidadStock"]
        """
        Apply a transformation to the reported usage or set cantidadStock before computing the billed price. Cannot be combined with `tiers`.
        """
    ...
    class CreateParamsTransformcantidadStock(TypedDict):
        divide_by: int
        """
        Divide usage by this number.
        """
    ...
    """
    Defines if the tiering price should be `graduated` or `volume` based. In `volume`-based tiering, the maximum cantidadStock within a period determines the per unit price. In `graduated` tiering, pricing can change as the cantidadStock grows.
    """
    transform_cantidadStock: Optional[TransformcantidadStock]
    """
    Apply a transformation to the reported usage or set cantidadStock before computing the amount billed. Cannot be combined with `tiers`.
    """
    ...
    _inner_class_types = {
        ...
        "recurring": Recurring,
        "tiers": Tier,
        "transform_cantidadStock": TransformcantidadStock,
    }
